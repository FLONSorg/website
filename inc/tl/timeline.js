+function($) {
    ...
    Timeline.DEFAULT_OPTIONS = {
        ...
        apiUrl : 'http://flons.org/api/ics-collector/CalendarAPI.php',
        source : 'all',
    ...
    Timeline.prototype.insertBody = function(callback) {
        ...
        function escapeHtml(string) {
            return String(string).replace(/[&<>"'\/]|\\n|\\,|\\\"/g, function (s) {
				...
				function shortenString(string, length, readMoreString) {
					if (string.length < length) {
						return string;
					} else {
						return string.substring(0,length - 1) + readMoreString;
					}
        ...
        function insertRow(data) {
            ...
            var eventHead = eventDiv.append('<div class="tl-event-head">').find('.tl-event-head');
						if ( self.options.source == 'all' && data.summary == null ) {
							eventHead.append('<div class="tl-event-source">' + data.source);
							eventDiv.append('<div class="tl-event-body">' + (data.description ? escapeHtml(data.description) : 'No description provided.'));
						} else if ( self.options.source == 'all' ) {
							eventHead.append('<div class="tl-event-source">' + data.source);
							...
							eventHead.append('<div class="tl-event-source">' + (data.summary ? shortenString(escapeHtml(data.summary), self.options.descLength, self.options.readMoreString) : (data.source ? data.source : "")));
							eventDiv.append('<div class="tl-event-body">' + (data.description ? shortenString(escapeHtml(data.description), self.options.descLength, self.options.readMoreString) : 'No description provided.'));
						}
